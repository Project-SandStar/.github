...
| [**mcp-proxy**](https://github.com/Project-SandStar/mcp-proxy) | A reconnecting proxy for MCP servers. Fronts any number of HTTP and stdio MCP servers behind a single port and holds the upstream connections itself, so when one restarts the client's session survives instead of dying with a "session not found" that fires no close event. | JavaScript · Node · MCP · PM2 |
